...
6. **Platform Abstraction.** Rust `CaptureEngine` trait decouples shared code
   ...
   isolated OBS Studio through the bundled `obs-cmd`; OS-specifics (paths,
...
├── src-tauri/
│   ├── binaries/                # obs (embedded OBS), obs-cmd, ffmpeg per triple
