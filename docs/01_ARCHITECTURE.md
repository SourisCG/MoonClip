...
- **Backend:** Rust + `tokio`, `serde`/`serde_json`, `rusqlite` (backend-only; `tauri-plugin-sql` was removed in Phase 2), `keyring`, `rodio`, `uuid`, `dirs`, `nix` (Linux), `image` (tray icon decode). Phase 4+: `sysinfo`; Phase 6: `oauth2`, `tiny_http`, `reqwest`; Windows: `cpal` + the bundled FFmpeg `gfxcapture` source (no capture crate).
- **Platform crates:**
  - Linux: `nix` (signals)
  - Windows: `cpal` (WASAPI capture/render), `windows` (DXGI monitor/vendor discovery, MMCSS, priorities)
...
│   │   │                        # windows/{engine,ts,audio,devices,video,caps,binary,open}
...
- While gaming: main window hidden to tray, WebView paused, capture in VRAM + encoded packets in RAM ring (`~100-150 MB` for 60s 1080p 15 Mbps). On Windows frames never leave the GPU (`gfxcapture` → NVENC/AMF/QSV, D3D11 zero-copy) and the save path is copy-only (live GPU scaling).
