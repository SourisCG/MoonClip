...
- **Backend:** Rust + `tokio`, `serde`/`serde_json`, `rusqlite` (backend-only; `tauri-plugin-sql` was removed in Phase 2), `keyring`, `rodio`, `uuid`, `dirs`, `nix` (Linux), `image` (tray icon decode). Phase 4+: `sysinfo`; Phase 6: `oauth2`, `tiny_http`, `reqwest`; Windows: `wasapi` (endpoint loopback + mic, QPC timestamps) + the bundled FFmpeg `gfxcapture` source (no capture crate).
- **Platform crates:**
  - Linux: `nix` (signals)
  - Windows: `wasapi` (WASAPI loopback/mic capture), `windows` (DXGI monitor/vendor discovery, MMCSS, priorities)
